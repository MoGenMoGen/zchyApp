<template>
	<!-- 全部投标 -->
	<div class="container">
		<van-dialog v-model="bail" title="保证金上传" show-cancel-button @cancel="cancelTo" @confirm='confirmTo'
	...
	<!-- 	<van-dialog v-model="bailDetail" title="保证金" show-cancel-button @cancel="cancelTo3" @confirm='confirmTo3'
		...
		<van-list v-model="loading" :finished="finished" finished-text="没有更多了" @load="getBidData"
			...
			<div class="bodyList" v-for="(item,index) in list" :key="index" @click="toDetail(item.bidId)">
				...
				<div class="listContent">
					...
					<div class="right" v-else-if="returnDate(1,item.bidOpenTm)" style="color: #2778BE;">
						评标中
					</div>
					<div class="right" v-else-if="returnDate(1,item.bidEndTm)" style="color: #3FAB2E;">
						投标中
					</div>
					<div class="right" v-else style="color: #E4393C;">
						待开标
					</div>
				</div>
				<div class="listContent">
	...
	export default {
		...
		methods: {
			...
			onBeforeClose(action, done) {
				console.log(this.imgListUpd);
				if (action === "confirm"&&this.depositAmt&&this.imgListUpd.length>0) {
					return done();
			...
			onBeforeClose2(action, done){
				if (action === "confirm"&&this.offerAmt&&this.fileListUpd.length>0) {
					return done();
				} else if(action === "cancel"){
					return done();
				}
