<template>
  <div class="hello">
    <!--<div @click="showData" class="preview"></div>-->
    <!-- 你不需要担心一个页面中过多vue-ueditor-wrap组件会导致混乱,或者重复引用JS(打开控制台瞄一眼),每个组件都保证拥有一个独立的UEditor实例,你可以尝试把数字改成99(根据自己电脑自行斟酌),但依然稳定可用 -->
    <vue-ueditor-wrap ref="ueditor" v-model="msgProps" :destroy="false" :config="config" @ready="ready" @beforeInit="addCustomUI"></vue-ueditor-wrap>
  </div>
</template>
...
export default {
  ...
  data () {
    return {
      ...
      config: {
        ...
        //  boot版接口
       // serverUrl: '/api/Eu/exec',
        // cloud版接口
        serverUrl: '/api/blade-resource/Eu/exec',
  ...
  watch: {
    // 当子组件的值改变时同时修改父组件的值
    msgProps: function () {
      this.$emit("changeEUditorContent",this.msgProps)
    }
  },
  methods: {
