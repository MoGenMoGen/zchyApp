...
<!--                    <img />-->
                    <van-image
                            ...
                            :src="info.coverImg"
                    />
                    <img :src="play" class="play" @click="toLook" v-if="type==1"/>
                ...
                <p class="ct1">{{info.name}}</p>
                <p class="ct2" v-html="info.anchorIntro"></p>
                <p class="ct3">{{info.startTime}}</p>
                <p class="last">
            ...
            <div class="whiteBox navBox">
                <p class="title"><span></span>直播简介</p>
                <p v-html="info.courseIntro" style="padding-bottom: 0.4rem;"></p>
            </div>
            ...
            <div class="whiteBox custBox">
                ...
                <div class="sort">
                    <div  v-for="(item,index) in list" :key="index" @click="livePlay(item)">
                        <img style="height: 100%;width: 100%;" :src="item.coverImg">
                        <p>{{item.name}}</p>
                    </div>
                </div>
            </div>
        ...
        <van-popup v-model="showBottom" position="bottom" :style="{ height: '30%' }" >
            <bottom-share :info="info" description="视频分享" :thumb="info.coverImg"></bottom-share>
        </van-popup>
    </div>
    ...
    export default {
		...
		methods: {
			...
			changeDevice(){
				...
				if(isPC){
					window.location.href = window.location.origin+this.serverAddr+'/learning/videoDetail?id='+this.id
				}
			},
            toShare(){
            ...
            toLook(){
                window.location.href = './lookLive.html?id='+this.id
            },
            livePlay(item){
                this.toPage('./detail.html?id='+item.id)
            },
            //推荐课程详情列表
            async getStudyContDetail(){
                let data = await this.api.livePlay(this.id)
                this.info = data.data
                this.$set(this.info,"nm",this.info.name)//用于分享的名称
                this.getStudyCont();
            },
            //收藏商品
            collectGoods(){
