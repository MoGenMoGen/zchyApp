...
<!--	资金管理-付款记录-->
    <div id="container">
        ...
        <pen-header title="选型记录"></pen-header>

<!--        <search-view place-text="时间区间" v-model="searchText" class="searchView"></search-view>-->
        <cal-range place-text="时间区间" v-model="searchText" class="searchView"></cal-range>
        ...
        <div class="body">
            ...
            <div class="list-cont">
                <van-list  v-model="loading" :finished="finished" finished-text="没有更多了"  @load="onLoad"   :immediate-check="immediate" >
                    <div class="list-item" v-for="item in list" :key="item.id" @click.stop="toPage(item)">
                        <van-row class="row" type="flex">
                            <van-col span="6"><p>船舶户籍:</p></van-col>
                            <van-col span="18"><p>{{item.registry}}</p></van-col>
                        </van-row>
                        <van-row class="row" type="flex">
                        ...
                        <van-row class="row" type="flex">
                            ...
                            <van-col span="18"><p>{{item.sailingAreaNm}}</p></van-col>
                        </van-row>
                        <van-row class="row" type="flex">
                            <van-col span="6"><p>船舶材质:</p></van-col>
                            <van-col span="18"><p>{{item.hullMaterialNm}}</p></van-col>
                        </van-row>
                        <van-row class="row" type="flex">
                        ...
                        <van-row class="row" type="flex" v-if="currentRole.identityCd=='identity20'">
                            <van-col span="5" ><p  class="btn-copy" @click.stop="toCopy(item)"><img :src="icCopy" >复制</p></van-col>
                            <van-col span="5" offset="2"><p  class="btn-edit" @click.stop="toEdit(item)"><img :src="icEdit" >编辑</p></van-col>
                            <van-col span="5" offset="2"><p  class="btn-look" @click.stop="toLook(item)"><img :src="icLook" >查看</p></van-col>
                        </van-row>

                        <p class="price" >
                            <span v-if="currentRole.identityCd=='identity20'">￥{{item.budget/1000 | numFilter}}万元</span>
<!--                       item.price的值是区间，     -->
                        ...
                        <van-icon name="arrow" class="arrow"  color="#666666"/>
                    </div>
                </van-list>
            </div>
        </div>
