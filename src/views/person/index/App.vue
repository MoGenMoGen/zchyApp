<template>
	<div class="person" :style="{backgroundImage:'url('+bg1+')'}">
		<div class="main">
			...
			<div class="infoBox">
				...
				<div>
					...
					<p>{{currentRole ? currentRole.company : info.company}}&nbsp;<span
							v-if="currentRole">[{{currentRole.identityNm}}]</span></p>
					<p @click="toPage('../person/persInfo.html')" class="updInfo">信息完善 ></p>
				</div>
			</div>
			<button class="changeIdent" @click="toPage('../home/changeUser.html')"> <i
					class="iconfont iconrefresh"></i>角色转换</button>
			<div class="whiteBox myCollBox">
				<p style="border-right: 0.01rem solid rgba(0,0,0,0.1);">
					<span class="num">{{collNum}}</span><span @click="until.href('../person/collect.html')">我的收藏</span>
				</p>
				<p>
					<span class="num">{{subscribeTotal}}</span>
					<span @click="until.href('../person/subscribe.html')"
						v-if="(!info.pid || permiList.findIndex(item=>item.nm==='我的预约'&&item.checked)>-1)">我的预约</span>
		...
		<van-popup v-model="show" closeable close-icon="close" position="center" style="width: 80%;height: 80%;border-radius: 0.1rem;">
			<div class="pop-title">{{tipsInfo.nm}}</div>
			<div class="pop-cont" v-html="tipsInfo.cont"></div>
		</van-popup>
				...
				toolList15: [ //供应商
					{
						...
						img: person17,
						nm: '销售订单',
						url: '../person/myOrder.html?orderType=6&item=' + JSON.stringify({
							id: 0,
							nm: '全部',
							statusCd: '01'
						}),
		...
		methods: {
			...
			async getPermiList() {
				...
				this.toolList1.forEach((item, index) => {
					console.log(item)
					let i = this.permiList.findIndex(v => v.nm === item.nm && !v
						.checked) //名字一样并且未勾选的没有权限 名字一样说明有权限区分，没有勾选说明当前账号无权限
					if (i > -1) {
			...
			getTips() {
			  this.api.ruleDetail('5724851888591872').then(res => {
