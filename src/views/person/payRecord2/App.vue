	...
	<div id="container">
		...
		<div class="body">
			...
			<div class="list-header">
				<ul
					v-if="identityCd=='identity10' || identityCd=='identity80' || identityCd=='identity90' || identityCd=='identity60'">
					<li v-for="item in tab1" @click="toChoose(item)"><span
							:class="{active:tabId==item.id}">{{item.nm}}</span></li>
				</ul>
				<!--设计院-->
				<ul v-if="identityCd=='identity20' ">
					<li v-for="item in tab2" @click="toChoose(item)"><span
							:class="{active:tabId==item.id}">{{item.nm}}</span></li>
				</ul>
				<!--                经销商-->
				<ul v-if="identityCd=='identity70' ">
					<li v-for="item in tab3" @click="toChoose(item)"><span
							:class="{active:tabId==item.id}">{{item.nm}}</span></li>
				</ul>

				<!--                检验检测机构-->
				<ul v-if="identityCd=='identity50' ">
					<li v-for="item in tab4" @click="toChoose(item)"><span
							:class="{active:tabId==item.id}">{{item.nm}}</span></li>
				</ul>
				<!--                监理建造机构-->
				<ul v-if="identityCd=='identity40' ">
					<li v-for="item in tab5" @click="toChoose(item)"><span
							:class="{active:tabId==item.id}">{{item.nm}}</span></li>
				</ul>
				<!--                供应商-->
				...
				<ul v-if="identityCd=='identity30' ">
					<li v-for="item in tab8" @click="toChoose(item)"><span
							:class="{active:tabId==item.id}">{{item.nm}}</span></li>
				</ul>
			...
			<div class="list-cont">
				<van-list v-model="loading" :finished="finished" finished-text="没有更多了" @load="onLoad"
					...
					<div v-if="tabId!=0">
						<div class="list-item" v-for="(item,index) in list" :key="index">
							<van-row class="row" align="center" type="flex" v-if="!((identityCd == 'identity10'||identityCd == 'identity80'||identityCd == 'identity90'||identityCd == 'identity70')&&tabId==1)">
								<van-col span="6">
									<p>订单编号：</p>
								</van-col>
								<van-col span="18">
							...
							<van-row class="row" align="center" type="flex" v-else>
								<van-col span="6">
									<p>合同编号：</p>
								</van-col>
								<van-col span="18">
	...
	export default {
		...
		methods: {
			...
			async getPermiList() {
				...
				if (this.identityCd == 'identity10' || this.identityCd == 'identity80' || this.identityCd ==
					'identity90') {
			...
			async getList() {
				...
				if (this.identityCd == 'identity10' || this.identityCd == 'identity80' || this.identityCd ==
					'identity90') { //船东，代理商，服务商
					if(this.tabId==0) {
						this.query.toW(qry,'payerId',this.currentRole.id+'','EQ')
						let param2 = {
						  type: 1
						}
						res=await this.api.payment3(this.query.toEncode(qry),param2)
					} else {
						this.query.toW(qry,'payerId',this.currentRole.id+'','EQ')
						let param2 = {
						  type: 0
					...
					res = await this.api.paymentList(this.query.toEncode(qry), param)
				}
				...
				this.loading = false;
				this.pageNo++
				console.log(this.pageNo,this.list)
			}
