            ...
            <div class="list-cont">
                <div class="list-item" >
                    <van-row class="row" type="flex">
                        ...
                        <van-col span="18"><p>{{info.contractNo}}</p></van-col>
                    </van-row>
                    <van-row class="row" type="flex">
                        <van-col span="6"><p>船舶名称:</p></van-col>
                        <van-col span="18"><p>{{info.nm}}</p></van-col>
                    </van-row>
                    <van-row class="row" type="flex">
                        <van-col span="6"><p>签约日期:</p></van-col>
                        <van-col span="18"><p>{{info.signTm}}</p></van-col>
                    </van-row>
                    <van-row class="row" type="flex">
                        <van-col span="6"><p>交付日期:</p></van-col>
                        <van-col span="18"><p>{{info.deliveryTm}}</p></van-col>
                    </van-row>
                    <van-row class="row" type="flex">
                    ...
                    <van-row class="row" type="flex">
                        ...
                        <van-col span="18"><p>{{info.installment}}</p></van-col>
                    </van-row>

                    <van-row class="row" type="flex">
                        <van-col span="6"><p>已付金额:</p></van-col>
                        <van-col span="18"><p>￥{{info.paid}}</p></van-col>
                    </van-row>

                    <van-row class="row" type="flex">
                        <van-col span="6"><p>未付金额:</p></van-col>
                        <van-col span="18"><p>￥{{info.totalPrice-info.paid}}</p></van-col>
                    </van-row>


                    <van-row class="row" type="flex">
                        <van-col span="6"><p>乙方公司:</p></van-col>
                        <van-col span="18"><p>{{info.partybNm}}</p></van-col>
                    </van-row>
                ...
                <div class="list-item" >
                    <van-row  class="row2" type="flex" align="center" v-for="(item,index) in attachList" :key="index">
                        <van-col span="6"><p>合同附件{{index+1}}:</p></van-col>
                        <van-col span="2" offset="1"><img :src="item.img"></van-col>
                        <van-col span="10" offset="1"><p>{{item.fileNm}}</p></van-col>
                        <van-col span="4"><p class="btn" @click="toDown(item)">下载</p></van-col>
                    </van-row>
                </div>
    ...
    export default {
		...
		methods: {
			...
			changeDevice(){
				...
				if(isPC){
					window.location.href = window.location.origin+this.serverAddr+'/personal/contractDetail?id='+this.id
				}
			},
		    //下载按钮
            toDown(item){
