...
<!--					<img :src="info.collected==1?started:start" @click="toCollect" class="startI">-->
				...
				<div class="infoBasic">
					...
					<p style="font-size: 0.24rem;color: #666666;margin-bottom: 0.3rem;">设计优势：{{info.business}}</p>
    ...
    mounted() {
		...
		methods: {
			...
			changeDevice(){
				...
				if(isPC){
					window.location.href = window.location.origin+this.serverAddr+'/shipDisplay/design?id='+this.id
				}
			},
			async getInfo(){
