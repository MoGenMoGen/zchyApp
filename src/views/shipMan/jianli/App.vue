<template>
    <div class="app">
			<van-nav-bar fixed title="建造监理" :border="false" class="fixTop">
			...
			<van-swipe class="homeLunBo" :autoplay="3000" indicator-color="white" style="margin: 0 0.2rem 0.2rem;">
			  <van-swipe-item v-for="(item,index) in jianliList" :key="index"><img :src="item.imgUrl" alt=""></van-swipe-item>
			</van-swipe>
		</div>
    </div>
    ...
    data() {
        ...
        methods: {
			async getAdert(){
				...
				this.jianliList=await this.api.getAdert('APPjljz')
			},
