<template>
  <div class="app">
		<van-nav-bar fixed title="建造船厂总览" :border="false" fixed class="fixTopAll">
    ...
    <div class="main">
			<div class="designBox" v-for="(item,index) in list" :key="index" @click="toDetail(item.id)">
				...
				<p class="infoP">
					...
					<span class="busS">建造优势: {{item.business}}</span>
    ...
    mounted() {
		...
		methods: {
			...
			changeDevice(){
				...
				if(isPC){
					window.location.href = window.location.origin+this.serverAddr+'/shipDisplay/allShipyard'
				}
			},
			async getList(){
			...
			toDetail(id){
				this.until.href("./shipYard.html?id="+id)
			}
    },
