<template>
	<div class="app">
		<van-nav-bar fixed :title=titleNm :border="false" fixed class="fixTop">
			<van-icon name="arrow-left" slot="left" color="#ffffff" style="font-size: 0.34rem;" @click="back">
			</van-icon>
		</van-nav-bar>
		<div class="main">
			<div class="lunboBox" :style="{backgroundImage:'url('+bgS+')'}">
				<van-swipe class="homeLunBo" :autoplay="3000" indicator-color="white">
					<van-swipe-item v-for="(item,index) in lunBoList" :key="index"><img :src="item" alt="">
					</van-swipe-item>
				</van-swipe>
			</div>
			<div class="whiteBox" v-if="list.length>0">
				<div class="part" v-for="(i,k) in list" :key="k" @click="toDetail(i.id)">
					<img :src="Vr" v-if="i.vrUrl" @click.stop="toVr(i.id)"
						style="width: 1.38rem;height: 0.43rem;opacity: 0.9;position: absolute;top: 0;left: 0;">
					<img :src="i.imgUrl" />
					<div class="ct">
						<p>{{i.nm}}</p>
						<p>{{i.rmks}}</p>
		...
		methods: {
			...
			changeDevice() {
				...
				if (isPC) {
					...
					if (this.until.getQueryString('cid2')) {
						window.location.href = window.location.origin + this.serverAddr + '/shipDisplay/shipShow?cId=' +
							this.cid2 + '&cNm=' + nm + '&cImg=' + img
					} else if (this.until.getQueryString('cid3')) {
						window.location.href = window.location.origin + this.serverAddr + '/shipDisplay/shipShow?cId3=' +
							this.cid3 + '&cNm=' + nm + '&cImg=' + img
					}
