...
<!--	我的船舶-->
    <div id="container">
        <pen-header title="跟进记录"></pen-header>
        ...
        <div class="body">
            <van-list  v-model="loading" :finished="finished" finished-text="没有更多了"  @load="onLoad"   :immediate-check="immediate" >
                <div class="list-item" v-for="item in list" :key="item.id">
                    ...
                    <van-row class="row"  align="center" type="flex">
                        <van-col span="6"><p >跟进内容：</p></van-col>
                        <van-col span="18"><p>{{item.cont}}</p></van-col>
                    </van-row>
                    <van-row class="row"  align="center" type="flex">
                        <van-col span="6"><p >跟进方式：</p></van-col>
                        <van-col span="18"><p>{{item.typesNm}}</p></van-col>
                    </van-row>
                    <van-row class="row"  align="center" type="flex">
                        <van-col span="6"><p >下次跟进时间：</p></van-col>
                        <van-col span="18"><p>{{item.nextFollowTm}}</p></van-col>
                    </van-row>
                    <van-row class="row"  align="center" type="flex">
                        <van-col span="6"><p >跟进状态：</p></van-col>
        ...
        <b-button title="新增跟进" @click="toAdd"></b-button>
    </div>
</template>
...
<script>
    ...
    export default {
		...
		methods: {
            ...
            toAdd(){
                this.until.href("./followAdd.html")
            },
		   async	getList(){
               ...
               if(this.searchText){
				...
				let res =await  this.api.myCustFollow(this.query.toEncode(qry))
