      ...
      <div class="main">
				...
				<div class="whiteBox" style="display: none">
					...
					<div class="contentBox" id="box" v-if="expressList.length>0">
						...
						<img :src="tradingR">  <!-- :style="{width:boxWidth+'rem'}" -->
					</div>
				</div>
				...
				<div class="whiteBox goodsList">
					...
					<div :class="info.ship=='1'?'shipBox':'proBox'">
						<div v-for="(item,index) in goodList" :key="index">
							<img :src="item.goodsImgUrl.split(',')[0]" v-if="item.goodsImgUrl">
    ...
    mounted() {
		...
		methods: {
			...
			changeDevice(){
				...
				if(isPC){
					window.location.href = window.location.origin+this.serverAddr+'/order/orderDetail?id='+this.id
				}
			},
			async getInfo(){
				...
				if(this.info.payTm){
				  ...
				  if(this.info.status=='10'){
				    this.info.statusNm='待支付'
					 if(this.info.offlineStatus == 1){
                         this.info.statusNm='线下支付待审核'
					 }else if(this.info.offlineStatus == 2){
                         this.info.statusNm='线下支付待审核通过'
					 }else if(this.info.offlineStatus == 3){
                         this.info.statusNm='线下支付待审核不通过'
                     }
                      if(this.info.stageStatus == 1){
                          this.info.statusNm='申请分期待审核'
                      }else if(this.info.stageStatus == 2){
                          this.info.statusNm='申请分期审核通过'
                      }else if(this.info.stageStatus == 3){
                          this.info.statusNm='申请分期审核不通过'
                      }
				  ...
				  if(this.info.payType==0){
					...
					// if(this.info.logiEntpCd&&this.info.logiCd){
					// 	this.getTrace()
					// }
			},
			//取消订单
			toCancel(){
				this.api.cancelPay(this.id).then(()=>{
				    this.$toast.success('取消成功');
				    this.getInfo()
				})
			},
			async getTrace(){//获取物流信息
				let param={
					com:this.info.logiEntpCd,
					num:this.info.logiCd
				}
				let data=await this.api.trace(param)
				let parse = JSON.parse(data.msg);
				if (parse.status === '200'){
				  this.expressList = parse.data;
					// this.boxWidth=document.getElementById('box').clientHeight
